...
public class IP {
	...
	public static String getMask() {
		if (IP.mask == null) {
			try {
				...
				Sys.error("Cannot create Mask", e);
				return null;
			}
		}
		return IP.mask;
	...
	public static String getBroadcast() {
		if (IP.broadcast == null) {
			try {
				...
				Sys.error("Cannot create Broadcast", e);
				return null;
			}
		}
		return IP.broadcast;
	...
	public static InetAddress getInetAddressFromIP(String ip) {
		try {
			...
			Sys.error("Cannot create inetaddress from ip [0]", e, ip);
			return null;
		}
	}
	...
	public static InetSocketAddress getSocketAddress(int port) {
		return IP.getSocketAddress(IP.getIP(), port);
	} 
	...
	public static String getConnectionIP(Socket socket) {
		String address = socket.getRemoteSocketAddress().toString().substring(1);
		
		return address.split(":")[0];
	}
	
	public static int getConnectionPort(Socket socket) {
		String address = socket.getRemoteSocketAddress().toString().substring(1);
